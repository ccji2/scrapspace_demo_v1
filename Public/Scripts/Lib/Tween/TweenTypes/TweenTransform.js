...
function startTween() {
    if (!global.tweenManager) {
        print(
            'Tween Transform: Tween Manager not initialized. Try moving the TweenManager script to the top of the Objects Panel or changing the event on this TweenType to "Lens Turned On".'
        );
        return;
    }
                ...
                script.api.start =
                    script.type == 2
                        ? quat.fromEulerAngles(
                              script.start.x * DEG_TO_RAD,
                              script.start.y * DEG_TO_RAD,
                              script.start.z * DEG_TO_RAD
                          )
                        : script.start;
                break;
            case 1:
                script.api.start =
                    script.type == 2
                        ? transformParameter
                        : new vec3(transformParameter.x, transformParameter.y, transformParameter.z);
                break;
            case 2:
                script.api.start =
                    script.type == 2
                        ? quat.fromEulerVec(script.from.uniformScale(DEG_TO_RAD))
                        : new vec3(script.from.x, script.from.y, script.from.z);
                break;
            case 3:
                script.api.start =
                    script.type == 2
        ...
        switch (script.movementType) {
            case 0:
                script.api.end =
                    script.type == 2
                        ? quat.fromEulerAngles(
                              script.end.x * DEG_TO_RAD,
                              script.end.y * DEG_TO_RAD,
                              script.end.z * DEG_TO_RAD
                          )
                        : script.end;
                break;
            case 1:
                script.api.end =
                    script.type == 2
                        ? quat.fromEulerVec(script.to.uniformScale(DEG_TO_RAD))
                        : new vec3(script.to.x, script.to.y, script.to.z);
                break;
            case 2:
                script.api.end =
                    script.type == 2
            ...
            case 3:
                script.api.end =
                    script.type == 2
                        ? transformParameter
                              .multiply(quat.angleAxis(script.offset.x * DEG_TO_RAD, vec3.right()))
                              .multiply(quat.angleAxis(script.offset.y * DEG_TO_RAD, vec3.up()))
                              .multiply(quat.angleAxis(script.offset.z * DEG_TO_RAD, vec3.forward()))
                        : new vec3(
                              transformParameter.x + script.offset.x,
                              transformParameter.y + script.offset.y,
                              transformParameter.z + script.offset.z
                          );
                break;
        }
    }
