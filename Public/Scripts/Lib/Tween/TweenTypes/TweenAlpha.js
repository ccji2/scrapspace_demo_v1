...
function startTween() {
    if (!global.tweenManager) {
        print(
            'Tween Alpha: Tween Manager not initialized. Try moving the TweenManager script to the top of the Objects Panel or changing the event on this TweenType to "Lens Turned On".'
        );
        return;
    }
...
function setupAlphaComponentTweens(componentType, sceneObject) {
    for (var i = 0; i < sceneObject.getComponentCount(componentType); i++) {
        ...
        if (visualComponent.getTypeName() == 'Component.Text') {
            setupTextProperties(visualComponent);
        }
...
function getVisualComponentAlpha(visual) {
    ...
    } else if(visual.getMaterial(0).getPass(0).baseOpacity != undefined) {
        color = visual.getMaterial(0).getPass(0).baseOpacity
    } else {
    ...
    if (color && color.a != undefined) {
        ...
        print(
            "TweenAlpha: Visual Component on Object '" +
                visual.getSceneObject().name +
                "' does not have a material supported by TweenAlpha. Setting alpha to 1.0."
        );
        return 1.0;
    }
}

function setupTextProperties(visualComponent) {
    // Adding properties to track starting outline and drop shadow colors
    visualComponent.textOutlineAlpha = visualComponent.outlineSettings.fill.color.a;
    visualComponent.textDropShadowAlpha = visualComponent.dropshadowSettings.fill.color.a;
}
...
function updateText(visualComponent, value) {
    ...
    outlineColor.a = visualComponent.textOutlineAlpha;
    ...
    dropShadowColor.a = visualComponent.textDropShadowAlpha;
...
function updateVisual(visualComponent, value) {
    ...
    if (currColor && currColor.a != undefined) {
        ...
        if (visualComponent.getMaterial(0).getPass(0).baseOpacity != undefined) {
            visualComponent.getMaterial(0).getPass(0).baseOpacity = currColor.a;
        }
    }
}
