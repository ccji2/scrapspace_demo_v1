...
TWEEN.Tween.prototype.resume = function(tweenName) {
    if (!this._isPaused) {
        debugPrint(
            'Tween Manager: ' + tweenName + ' has not been paused. Did you mean to call startTween this Tween instead?'
        );
        return;
    }
        ...
        if (tweenScriptComponent.api.tweenType == 'chain') {
            if (completeCallback) {
                if (tweenScriptComponent.api.playAll) {
                    ...
                    if (Array.isArray(tweenScriptComponent.api.lastTween)) {
                        tweenScriptComponent.api.lastTween[tweenScriptComponent.api.lastTween.length - 1].onComplete(
                            completeCallback
                        );
                    } else {
            ...
            if (startCallback) {
                if (Array.isArray(tweenScriptComponent.api.firstTween)) {
                    tweenScriptComponent.api.firstTween[tweenScriptComponent.api.firstTween.length - 1].onStart(
                        startCallback
                    );
                } else {
            ...
            if (completeCallback) {
                if (Array.isArray(tweenScriptComponent.api.tween)) {
                    tweenScriptComponent.api.tween[tweenScriptComponent.api.tween.length - 1].onComplete(
                        completeCallback
                    );
                } else {
...
function getGenericTweenValue(tweenObject, tweenName) {
    for (var i = 0; i < tweenObject.getComponentCount('Component.ScriptComponent'); i++) {
        ...
        if (scriptComponent.api) {
                ...
                if (scriptComponent.api.tweenType == 'value') {
                    if (scriptComponent.api.tween) {
                        if (!scriptComponent.api.tween._isPlaying) {
                            debugPrint(
                                'Tween Manager: Tween Value, ' +
                                    tweenName +
                                    ', is not currently playing. Ensure that it has been started by either calling its startTween() function or by setting it to Play Automatically.',
                                true
                            );
                        }
                    } else {
                        debugPrint(
                            'Tween Manager: Tween Value, ' +
                                tweenName +
                                ', has not been set up. Ensure that this Tween Value is ordered before every other script that uses it in the Objects Panel and Inspector. Try initializing it in the Initialized event and scripting in the Lens Turn On event.',
                            true
                        );
                    }
            ...
            debugPrint(
                'Tween Manager: Tween Value, ' +
                    tweenName +
                    ", hasn't initialized. Needs to initialize prior to scripting playback. Likely an order of operations issue. Try initializing tween type in the Initialized event and scripting it in the Lens Turn On event. Or, try moving the Tween Manager to the top of the Objects hierarchy.",
                true
            );
            return;
        }
    ...
    debugPrint(
        'Tween Manager: Tween Value, ' +
            ...
            ', is not found. Ensure that ' +
            tweenName +
            ' is enabled and the Tween Name passed into this function exactly matches the specified Tween Name for this Tween Value in the Inspector.',
        true
    );
}
